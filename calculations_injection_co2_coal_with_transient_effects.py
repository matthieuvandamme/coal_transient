...
# Parameters of coal
REF_PERM_OF_CLEATS = 1E-12 # m2, ref. perm. of coal rock in unstressed state
# choose between 1E-13, 1E-12, 1E-11
# Parameters of interaction between fluid and coal
TYPE_OF_KERNEL = 'fitted' # choose between 'simulated' and 'fitted'

# Parameters of computation
NUM_OF_ELEMENTS = 50
NUM_OF_NODES = NUM_OF_ELEMENTS + 1
if (REF_PERM_OF_CLEATS == 1E-11):
    time_step = 10.0 # s
    NUM_OF_STEPS = 5000
    TYPE_OF_TIME_STEP = 'equispaced' # older version made it possible to use increasing time steps
if (REF_PERM_OF_CLEATS == 1E-12):
    time_step = 20.0 # s
    NUM_OF_STEPS = 5000
    TYPE_OF_TIME_STEP = 'equispaced'
if (REF_PERM_OF_CLEATS == 1E-13):
    time_step = 20.0 # s
    NUM_OF_STEPS = 10000
    TYPE_OF_TIME_STEP = 'equispaced'
LENGTH = 200 # m
...
def density_co2(p):
    # return MOLAR_MASS_OF_CO2 * p / (IDEAL_GAS_CONSTANT * TEMPERATURE) # for an ideal gas
    return MOLAR_MASS_OF_CO2 * (p / (IDEAL_GAS_CONSTANT * TEMPERATURE) + 
                                21887 * (1 / (1 + np.exp(-(p-9918656)/1212276)))) # phenomenological fit on NIST EOS at 40°C between 1MPa and 10MPa
            
# kg/m3/Pa, returns the derivative of the density with respect to pressure p
def diff_density_co2(p):
    # return MOLAR_MASS_OF_CO2 / (IDEAL_GAS_CONSTANT * TEMPERATURE) * p / p # for an ideal gas
    return MOLAR_MASS_OF_CO2 * (1 / (IDEAL_GAS_CONSTANT * TEMPERATURE) * p / p
                                + 21887 / 1212276 * np.exp(-(p-9918656)/1212276) /
                                np.power(1 + np.exp(-(p-9918656)/1212276), 2))
...
# We consider that the initial system was fully equilibrated at the initial pressure
...
# m_m is divided by (1-REF_POROSITY) to express the adsorbed amount per unit volume of coal matrix
