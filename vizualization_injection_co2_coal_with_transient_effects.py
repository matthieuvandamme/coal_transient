...
NPZ_FILES_TO_ANALYZE =['perm1E-19_simulated_equispaced.npz',
                       'perm1E-18_simulated_equispaced.npz',
                       'perm1E-17_simulated_equispaced.npz',
                       'perm1E-18_fitted_equispaced.npz']
...
for i in NPZ_FILES_TO_ANALYZE:
    ...
    if (ref_permeability == 1E-19):
        times_to_display = [0, 10000, 30000, 50000, 100000, 200000, 300000, 1000000]
        times_to_display = [0, 10000, 30000, 50000, 100000, 150000, 200000, 300000]
    if (ref_permeability == 1E-18):
        times_to_display = [0, 300, 1000, 3000, 10000, 30000, 100000]
    if (ref_permeability == 1E-17):
        times_to_display = [0, 10, 30, 100, 300, 1000, 3000, 10000, 30000, 100000]
    indices_to_display = find_indices(times_to_display, times)
    f, ax = plt.subplots()
    for i in indices_to_display:
        plt.plot(positions*1E3, pressure[:, i]/1E6, '-o', markeredgecolor = 
                 'black', label = '$t$ = {:.0f} s'.format(times[i]))
    plt.xlabel('Position, mm')
    plt.ylabel('Pressure, MPa')
    plt.gca().set_xlim(left=0, right = positions[-1]*1E3)
#    plt.gca().set_ylim(bottom=3)
    ...
    plt.figure()
    for i in indices_to_display:
        plt.plot(positions*1E3, langmuir_isotherm(pressure[:, i]), '-o', 
                 markeredgecolor = 'black')
    plt.gca().set_prop_cycle(None)
    for i in indices_to_display:
        plt.plot(positions*1E3, adsorbed_amount[:, i] + langmuir_isotherm(
            pressure[-1,0]), '-',
            label = '$t$ = {:.0f} s'.format(times[i]))
    plt.xlabel('Position, mm')
    plt.ylabel('Amount of fluid adsorbed on edge \n of cleat or in coal matrix, kg/m$^3$')
    plt.gca().set_xlim(left=0, right = positions[-1]*1E3)
#    plt.gca().set_ylim(bottom=75)
    plt.legend(fontsize = 12, ncol = 2, loc = 'lower right')
    plt.show()
    ...
    if (ref_permeability == 1E-19):
        times_to_display = [0, 100, 300, 1000, 3000, 5000, 10000, 20000, 50000,
                            100000, 150000, 200000, 300000, 1000000]
        times_to_display = [0, 100, 300, 1000, 3000, 5000, 10000, 20000, 50000,
                            100000, 150000, 200000, 300000]
    if (ref_permeability == 1E-18):
        times_to_display = [0, 25, 50, 100, 200, 500,
                            1000, 2000, 5000, 10000, 20000,
                            40000, 60000, 100000, 250000]
        times_to_display = [0, 25, 50, 100, 200, 500,
                            1000, 2000, 5000, 10000, 20000,
                            40000, 60000, 100000]
    if (ref_permeability == 1E-17):
        times_to_display = [0, 10, 20, 30, 50, 70, 100, 200, 500,
                            1000, 2000, 5000, 10000, 20000,
                            40000, 100000]
    indices_to_display = find_indices(times_to_display, times)
    plt.figure()
    for i in indices_to_display:
        plt.plot(positions*1E3, permeability[:, i], '-o', markeredgecolor = 
                 'black', label = '$t$ = {:.0f} s'.format(times[i]))
    plt.xlabel('Position, mm')
